@props(['id', 'title' => "", 'xdata' => "{}", 'header' => 'true', 'closable' => 'true', 'placement' => 'center', 'backdrop' => 'dynamic', 'backdropClasses' => 'bg-gray-900 bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-40'])
        ...
        <div class="relative bg-gray-50 rounded-lg shadow dark:bg-gray-700">
            ...
            @if ($header == "true")
                <div class="flex items-start justify-between p-4 border-b border-gray-300 rounded-t dark:border-gray-600">
                    <h3 x-text="title" class="text-xl font-semibold text-black dark:text-white">
                    </h3>
        ...
        Object.assign(modals.{{$id}},{...modals.{{$id}}.__proto__,
            initData:()=> {
                return { 
                    modalID: '{{ $id }}', 
                    title: "{{$title}}",
                    // init(){console.log("samurai");Alpine.store('modals').{{ $id }} = {
